...
//!
//! Tolerance calibration (2024-02 baseline):
//!   Q4_0 quantization + different matmul/accumulation order produces systematic
//!   divergence from llama.cpp. Current tolerances are wide to serve as a regression
//!   guard. Tighten as implementation improves.

use rustml_core::Tensor;
use rustml_gguf::GGUFFile;
use rustml_nlp::{
    convert_tensors, gguf_config_to_model_config, GgufTokenizer, LanguageModel, LlmModel,
...
/// Calibrated: plain_text=8, chat_template=7, short_input=11 (2026-02 baseline).
const MIN_TOP20_OVERLAP: usize = 5;

/// Logit absolute tolerance for value comparison.
/// Calibrated: max observed diff is ~36 on chat_template (2026-02 baseline).
const LOGIT_ABS_TOLERANCE: f32 = 40.0;

/// Relative tolerance for logit statistics.
/// Calibrated: max observed rel_err is ~0.67 on chat_template max (2026-02 baseline).
const STAT_RELATIVE_TOLERANCE: f64 = 0.70;
...
fn verify_logits_match_reference() {
    ...
    for prompt_data in prompts {
        ...
        let mut our_token_ids = vec![bos_id];
        our_token_ids.extend(tokenizer.encode(prompt_text).expect("Tokenization failed"));
