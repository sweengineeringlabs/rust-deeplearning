...
fn run_verify(path: &PathBuf) -> Result<()> {
    ...
    if let Some(info) = embd {
        if info.dimensions.len() == 2 && info.dimensions[0] == config.dim {
            println!(
                "PASS  token_embd.weight shape [{}x{}] matches dim={}",
                info.dimensions[0], info.dimensions[1], config.dim
            );
            passes += 1;
        } else {
            println!(
                "FAIL  token_embd.weight shape {:?} — expected dim[0]={}",
    ...
    if let Some(info) = embd {
        if info.dimensions.len() == 2 {
            ...
            if embd_vocab == config.vocab_size {
                println!(
                    "PASS  vocab_size={} matches token_embd.weight[1]",
                    config.vocab_size
                );
                passes += 1;
            } else {
                println!(
                    "FAIL  vocab_size={} != token_embd.weight[1]={}",
                    config.vocab_size, embd_vocab
                );
                fails += 1;
            }
        }
    ...
    if let Some(info) = output {
        ...
        {
            println!(
                "PASS  output.weight shape [{}x{}]",
                info.dimensions[0], info.dimensions[1]
            );
            passes += 1;
        } else {
            println!(
                "FAIL  output.weight shape {:?} — expected [{}x{}]",
                info.dimensions, config.dim, config.vocab_size
            );
            fails += 1;
        }
    }
    ...
    if layer_ok {
        println!(
            "PASS  all layer tensors present for {} layers ({} suffixes each)",
            config.n_layers,
            LAYER_SUFFIXES.len()
        );
        passes += 1;
        ...
        for suffix in &["attn_q.weight", "attn_output.weight"] {
            ...
            if let Some(info) = find_tensor(&name) {
                if info.dimensions.is_empty() || info.dimensions[0] != config.dim {
                    println!(
                        "FAIL  {name} dim[0]={} != embedding_dim={}",
                        info.dimensions.first().unwrap_or(&0),
                        config.dim
                    );
                    fails += 1;
                    dim_ok = false;
...
pub fn run(command: GgufCommand) -> Result<()> {
    match &command {
        GgufCommand::Info { path } => {
            let gguf = GGUFFile::parse_header(path)
                .with_context(|| format!("Failed to parse GGUF: {}", path.display()))?;
