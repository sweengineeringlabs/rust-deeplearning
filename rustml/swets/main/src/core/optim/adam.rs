...
/// gradient penalty (i.e. the gradient is modified to include `weight_decay * param`
/// before computing the moment estimates). For decoupled weight decay, see `AdamW`.
pub struct Adam {
    learning_rate: f32,
    beta1: f32,
    beta2: f32,
    ...
    fn step(&mut self, params: &mut [&mut Tensor]) -> SwetsResult<()> {
        ...
        for param in params.iter_mut() {
            if let Some(mut grad) = tape::grad(param) {
                ...
                // L2 weight decay: add weight_decay * param to the gradient.
