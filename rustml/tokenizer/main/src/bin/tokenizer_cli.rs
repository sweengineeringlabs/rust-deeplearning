...
/// Read input text: from positional arg, --file, or stdin.
...
/// Read token IDs: from positional args, --file, or stdin.
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    let tokenizer = load_backend(&cli.backend)?;

    match &cli.command {
        Command::Encode { text, file, json } => {
